...
public interface ContractAPI {
    ...
    @ApiOperation(value = "Find all contract data",
            notes = "Returns a list of ContractDTO",
            response = DefaultResponseContractDTO.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Successful operation", response = DefaultResponseContractDTO.class)
    })
    @ApiImplicitParams({
            @ApiImplicitParam(name = "queryParameters", value = "page/size/sortField/sortOrder=ASC,DESC,NONE/filter")
    })
    @GetMapping(value = "/contract")
    ResponseEntity<DefaultResponse<ContractDTO>> findAll(@RequestParam Map<String, String> queryParameters);
    ...
    ResponseEntity<DefaultResponse<ContractDTO>> save(@Valid @NotNull @RequestBody ContractDTO contractDTO,
    ...
    ResponseEntity<DefaultResponse<ContractDTO>> update(@Valid @NotNull @RequestBody ContractDTO contractDTO,
                                                        BindingResult bindingResult);
    ...
    @ApiOperation(value = "Find contract content by ID",
    ...
    ResponseEntity<InputStreamResource> findContentByContractId(@PathVariable("id") Integer id);

    @ApiOperation(value = "Save content for contract by ID",
            notes = "Saves content for ContractDTO",
            response = Boolean.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Successful operation", response = Boolean.class)
    })
    @PostMapping(value = "/contract/{id}/content")
    ResponseEntity<DefaultResponse<ContractDTO>> saveContentByContractId(@PathVariable("id") Integer id,
