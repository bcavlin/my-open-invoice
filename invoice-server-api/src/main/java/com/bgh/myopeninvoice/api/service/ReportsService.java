...
public class ReportsService implements CommonService<ReportsEntity> {
    ...
    public Predicate getPredicate(SearchParameters searchParameters) {
        ...
        if (searchParameters.getFilter() != null) {
            builder.andAnyOf(
                    ...
                    QReportsEntity.reportsEntity.dateCreated.stringValue().contains(searchParameters.getFilter())
            );
    ...
    public List<ReportsEntity> findAll(SearchParameters searchParameters) {
        ...
        if (searchParameters.getPageRequest() != null) {
            if (predicate != null) {
                entities = Utils.convertIterableToList(reportsRepository.findAll(predicate, searchParameters.getPageRequest()));
            } else {
                entities = Utils.convertIterableToList(reportsRepository.findAll(searchParameters.getPageRequest()));
            }
        } else {
            if (predicate != null) {
